                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" id="tanggal_pembelian" class="form-control date" name="tanggal_pembelian"
                                ...
                                value="{{ date('d/m/Y', strtotime($purchase->tanggal_pembelian)) }}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" id="nomor_ref" class="form-control" name="nomor_ref"
                                placeholder="Nomor Ref." autocomplete="off" value="{{ $purchase->no_ref }}">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <fieldset class="form-group">
                            ...
                            <select class="form-select" id="status" name="status" disabled>
                                <option value="dibayar" {{ $purchase->status == 'dibayar' ? 'selected' : '' }}>
                                    Dibayar
                                </option>
                                <option value="sebagian" {{ $purchase->status == 'sebagian' ? 'selected' : '' }}>
                                    Sebagian
                                </option>
                                <option value="belum_dibayar" {{ $purchase->status == 'belum_dibayar' ? 'selected' : '' }}>
                                    Belum Dibayar
                                </option>
                            </select>
                        </fieldset>
                        ...
                        <table class="table" id="daftar-produk">
                            ...
                            <tbody>
                                ...
                                @foreach ($purchase->product_purchases as $product_purchase)
                                    @php
                                        $daftarProduk[] = [
                                            'id' => $product_purchase->product_id,
                                            'variation_id' => $product_purchase->product_variation_id,
                                            'name' => $product_purchase->product->name,
                                            'harga_beli' => $product_purchase->harga_beli,
                                            'jumlah' => $product_purchase->qty,
                                            'subtotal' => $product_purchase->qty * $product_purchase->harga_beli,
                                        ];
                                    @endphp
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            <input type="text" name="harga_beli[]"
                                                ...
                                                value="{{ number_format($product_purchase->harga_beli) }}">
                                        </td>
                                        ...
                                        <td class="text-end">
                                            ...
                                            <input type="hidden" name="variation_id[]"
                                                value="{{ $product_purchase->variation_id }}">
                                            <input type="hidden" class="input-subtotal" name="subtotal[]"
                                                value="{{ $product_purchase->qty * $product_purchase->harga_beli }}">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <th class="text-end" id="total-harga-beli">
                                        {{ number_format($purchase->total_harga_beli) }}
                                    </th>
                ...
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <input type="hidden" name="total_harga_beli" id="total_harga_beli"
                            value="{{ $purchase->total_harga_beli }}">
                        <input type="hidden" name="total_subtotal" id="total_subtotal" value="{{ $purchase->total }}">

                        <div class="form-group">
                            <label for="catatan">Catatan</label>
                            <textarea class="form-control" placeholder="Catatan" id="catatan" name="catatan" rows="10">{{ $purchase->catatan }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <fieldset class="form-group">
                        ...
                        @php
                            $jumlahBayar = 0;
                            foreach ($purchase->transactions as $trx) {
                                $jumlahBayar += $trx->total;
                            }
                        @endphp
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="jumlah_bayar" class="form-control text-end input-number"
                                name="jumlah_bayar" autocomplete="off" readonly="true"
                                value="{{ number_format($jumlahBayar) }}">
                        </div>
                    </div>
                </div>
...
@section('script')
    <script>
        var daftarProduk = @json($daftarProduk);
